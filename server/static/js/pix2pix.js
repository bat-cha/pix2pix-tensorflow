...
function main() {
    ...
    create_editor({
        name: "edges2beach",
        weights_url: "/models/edges2beach_AtoB.pict",
        mode: "line",
        clear: "#FFFFFF",
        colors: {
            line: "#005EB8",
        ...
        draw: "#005EB8",
        initial_input: "/images/edges2beach-input.png",
        initial_output: "/images/edges2beach-output.png",
        sheet_url: "/images/edges2beach-sheet.jpg",
    })

    create_editor({
        name: "edges2airliner",
        weights_url: "/models/edges2airliner_AtoB.pict",
        mode: "line",
        clear: "#FFFFFF",
        colors: {
            line: "#005EB8",
        ...
        draw: "#005EB8",
        initial_input: "/images/edges2airliner-input.png",
        initial_output: "/images/edges2airliner-output.png",
        sheet_url: "/images/edges2airliner-sheet.jpg",
    })
    ...
    render: function() {
        ...
        if (v.contains(mouse_pos)) {
            ...
            if (this.config.mode == "line" && this.config.draw == "#ffffff") {
                // eraser tool
                cursor_style = "url(/images/eraser.png) 8 8, auto"
            }
        }
        ...
        if (request_in_progress) {
            ...
            if (do_button(v, "process")) {
                ...
                sendGaEvent('Click', 'process', this.config.name, 1)
                update()
            })
            })
            },
        ...
        v.frame("undo_button", 192 - 32, 310, 64, 40, () => {
            if (do_button(v, "undo")) {
            this.pop_buffer()
            sendGaEvent('Click', 'undo', this.config.name, 1)
            update()
        }
    })
        ...
        v.frame("clear_button", 270 - 32, 310, 64, 40, () => {
            ...
            sendGaEvent('Click', 'clear', this.config.name, 1)
        }
    })
        ...
        if (this.sheet_loaded) {
            v.frame("random_button", 347 - 32, 310, 64, 40, () => {
                ...
                sendGaEvent('Click', 'random', this.config.name, 1)
                update()
            }
        })
        }
        ...
        v.frame("save_button", 655 - 32, 310, 64, 40, () => {
            ...
            sendGaEvent('Click', 'save', this.config.name, 1)
        }
    })
...
function sendGaEvent(category, action, label, value) {
    if ("ga" in window) {
        ...
        if (tracker)
            tracker.send('send', 'event', category, action, label, value);
    }
}
...
View.prototype = {
    ...
    relative: function(pos) {
        ...
        return {
            x: pos.x - rect.left - this.f.offset_x,
            y: pos.y - rect.top - this.f.offset_y
        }
    },
...
function do_button(v, text) {
    ...
    if (request_in_progress) {
        ...
        if (v.contains(mouse_pos)) {
            ...
            animate(name, parse_color("#005EB8FF"), 100)
        }
    }
